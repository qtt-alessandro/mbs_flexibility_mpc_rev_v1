...
def step_upper_level(horizon, prices_values, co2_progn_values, inflow_values, h_init, energy_init, Qout_init):
    ...
    tvar = opti.variable(1)
    si = opti.variable(horizon)
    ...
    opti.subject_to(lam * theta + (1/horizon) * ca.sum1(si) <= alpha * tvar)
    ...
    for t in range(1, horizon):
        ...
        objective +=  w1*(da_prices[t] * energy[t]) + w1*(co2_progn[t] * energy[t]) + 1e-3*(energy[t] - energy[t-1])**2 + 10*si[t] + 10*tvar#+ w2*s_h[t] 

        #opti.subject_to(Qout[t] ==  2.107*energy[t] + 0.8032* Qout[t-1])
        opti.subject_to(Qout[t] ==  4.64*energy[t] + 0.5336* Qout[t-1])


        volume_balance = Qout[t] - Qin_forecast[t] + tvar - np.random.normal(30, 2, 1)
        opti.subject_to(volume_balance <= si[t])
        
        #opti.subject_to(height[t] == height[t-1] + (100/40)*(Qin_forecast[t] + e_qin[t]  - Qout[t]))
        #opti.subject_to(height[t] == height[t-1] + (100/40)*(Qout[t] - Qin_forecast[t] + tvar - e_qin[t]))
        opti.subject_to(height[t] == height[t-1] + (100/40)*(volume_balance))
    ...
    try:
        ...
        print(np.round(opti.debug.value(volume_balance),1))  
