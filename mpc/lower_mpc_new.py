...
class LowerMPC:
    ...
    def step(self, ini_dict, inflow, reference):
        ...
        for k in range(0, N):
            constraints += [
                # Power ARX
                power_p1[k+1] == self.a_p1_power * power_p1[k] + self.b_p1_power * u1[k],
                power_p3[k+1] == self.a_p3_power * power_p3[k] + self.b_p3_power * u3[k],
                power_p4[k+1] == self.a_p4_power * power_p4[k] + self.b_p4_power * u4[k],
 
                # Qout ARX
                qout_p1[k+1] == self.a_p1_qout * qout_p1[k] + self.b_p1_qout * u1[k],
                qout_p3[k+1] == self.a_p3_qout * qout_p3[k] + self.b_p3_qout * u3[k],  
                qout_p4[k+1] == self.a_p4_qout * qout_p4[k] + self.b_p4_qout * u4[k],
 
                
                h[k+1] == h[k] + 100*(inflow[k] - qout_p1[k])/(40*60),
 
 
                u1 >= 0, u1 <= 1500,
                u3 >= 0, u3 <= 1500,
                u4 >= 0, u4 <= 1500,
                power_p1 >= 0, power_p1 <= 90,
                power_p3 >= 0, power_p3 <= 90,
                power_p4 >= 0, power_p4 <= 90,
